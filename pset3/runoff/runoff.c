...
bool vote(int voter, int rank, string name)
{
    // Check if name is in candidate, if so update vote and record preference 
    for (int i = 0; i < candidate_count; i++)
    {
...
void tabulate(void)
{
    // loop over preferance of each candidate and get preferance
    for (int i = 0; i < voter_count; i++)
    {
...
bool is_tie(int min)
{
    // Check if all candidates have min value
    for (int i = 0; i < candidate_count; i++)
    {
        if (!candidates[i].eliminated && candidates[i].votes != min)
        {
